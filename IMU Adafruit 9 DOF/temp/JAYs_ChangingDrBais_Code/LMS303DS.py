...
class LMS303DS(object):
    ...
    def __init__(self, busId, slaveAddr, ifLog, ifWriteBlock):
        ...
        bus = self.__i2c
        whoami = bus.read_byte_data(slaveAddr, WHO_AM_I)

        if(whoami == 0x49):
            bus.write_byte_data(LSM303D_AADDR, CTRL_REG1, 0x57) # 0x57 = ODR=50hz, all accel axes on ## maybe 0x27 is Low Res?
            bus.write_byte_data(LSM303D_AADDR, CTRL_REG2, (3<<6)|(0<<3)) # set full scale +/- 2g
            bus.write_byte_data(LSM303D_AADDR, CTRL_REG3, 0x00) # no interrupt
            bus.write_byte_data(LSM303D_AAADDR, CTRL_REG4, 0x00) # no interrupt
            bus.write_byte_data(LSM303D_AADDR, CTRL_REG5, (4<<2)) # 0x10 = mag 50Hz output rate
    ...
    def getMag(self):
        bus = self.__i2c
        self.mag[X] = twos_comp(bus.read_byte_data(LSM303D_MADDR, OUT_X_H_M) << 8 | 
                          bus.read_byte_data(LSM303D_MADDR, OUT_X_L_M), 16)
        self.mag[Y] = twos_comp(bus.read_byte_data(LSM303D_MADDR, OUT_Y_H_M) << 8 | 
                          bus.read_byte_data(LSM303D_MADDR, OUT_Y_L_M), 16)
        self.mag[Z] = twos_comp(bus.read_byte_data(LSM303D_MADDR, OUT_Z_H_M) << 8 | 
                          bus.read_byte_data(LSM303D_MADDR, OUT_Z_L_M), 16)

    def getAccel(self):
        bus = self.__i2c
       
        accel = [0,0,0]
        accel[X] = twos_comp(bus.read_byte_data(LSM303D_AADDR, OUT_X_H_A) << 8 | 
                           bus.read_byte_data(LSM303D_AADDR, OUT_X_L_A), 16)
        accel[Y] = twos_comp(bus.read_byte_data(LSM303D_AADDR, OUT_Y_H_A) << 8 | 
                           bus.read_byte_data(LSM303D_AADDR, OUT_Y_L_A), 16)
        accel[Z] = twos_comp(bus.read_byte_data(LSM303D_AADDR, OUT_Z_H_A) << 8 | 
                           bus.read_byte_data(LSM303D_AADDR, OUT_Z_L_A), 16)
    ...
    def isMagReady(self):
        bus = self.__i2c
        temp =  bus.read_byte_data(LSM303D_MADDR, STATUS_REG_M) & 0x03 & 0x03
